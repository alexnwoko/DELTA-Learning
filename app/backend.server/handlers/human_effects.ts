...
export async function loadData(
	...
	let categoryPresence = await categoryPresenceGet(
		dr,
		recordId,
		countryAccountsId,
		tblId,
	...
	let categoryPresenceTotals = await getTotalPresenceTable(
		dr,
		tblId,
		recordId,
		defs,
	...
	if (d.data.deletes || d.data.updates || d.data.newRows) {
		if (!d.columns) {
			return Response.json(
				{
					ok: false,
					error: `when passing data, columns are also required: ${expectedCols}`,
				},
				{ status: 400 },
			);
		}
	...
	try {
		...
		await dr.transaction(async (tx) => {
			...
			if (d.data.totalGroupFlags) {
				...
				let res = await calcTotalForGroup(
					tx,
					d.table,
					recordId,
					defs,
					d.data.totalGroupFlags,
				);
